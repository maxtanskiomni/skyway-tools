...
const MetricCard = ({ title, value, subtitle, color, loading = false }) => (
...
const TransactionTable = ({ transactions, columns, loading = false }) => {
  if (loading) {
    return (
      <Box sx={{ p: 2 }}>
        {[...Array(5)].map((_, index) => (
...
const TransactionAccordion = ({ title, amount, transactions, columns, loading = false }) => (
  <Accordion>
...
const DepartmentSection = ({ title, items, loading = false }) => (
  <Box sx={{ mb: 2.5 }}>
...
export default function PerformanceDashboard({ data, loading = false }) {
  // Check if we have any data to determine loading states
  ...
  const shippingLoads = data?.filter(x => x.type === "shipping_load") || [];
  ...
  const shippingLoadsRevenue = shippingLoads.reduce((sum, load) => sum + (load.revenue || 0), 0);
  const shippingLoadsProfit = shippingLoads.reduce((sum, load) => sum + (load.profit || 0), 0);
  ...
  // Calculate total gross profit and cash flow (including shipping loads)
  const totalGrossProfit = salesProfit + serviceProfit + shippingProfit + shippingLoadsProfit + financeProfit;
  ...
  const shippingCount = shipping.length + shippingLoads.length;
  ...
  const shippingCashFlow = shippingProfit + shippingLoadsProfit - shippingTransfers;
  ...
  const departmentSections = [
    {
      ...
      items: [
        {
          title: `Trades (${sales.filter(deal => (deal.trades || []).length > 0).length} transactions)`,
          amount: -salesTrades,
    ...
    },
    {
      ...
      items: [
        ...
        {
          title: `Shipping Loads (${shippingLoads.length} transactions)`,
          amount: shippingLoadsProfit,
          transactions: shippingLoads.sort((a, b) => moment(a.date).diff(moment(b.date))).map(load => ({
            date: load.date,
            stock: load.stock,
            car_count: load.car_count,
          ...
          columns: [
            { key: 'date', label: 'Date' },
            { key: 'stock', label: 'Load ID' },
            { key: 'car_count', label: 'Cars' },
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <MetricCard
                ...
                value={formatCurrency(shippingRevenue + shippingLoadsRevenue)}
            ...
            <Grid item xs={12} sm={6}>
              <MetricCard
                ...
                value={formatCurrency(shippingProfit + shippingLoadsProfit)}
                subtitle={`${formatCurrency(shippingCount > 0 ? (shippingProfit + shippingLoadsProfit) / shippingCount : 0)} per shipment`}
                loading={isLoading}
              />
            </Grid>
            <Grid item xs={12}>
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <Box sx={{ pl: 3 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
                        ...
                        <Typography variant="body1" color="text.primary" sx={{ textAlign: 'right' }}>
                          {isLoading ? <Skeleton variant="text" width={80} /> : formatCurrency(salesProfit)}
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
                        <Typography variant="body1" sx={{ textAlign: 'left' }}>Service</Typography>
                        <Typography variant="body1" color="text.primary" sx={{ textAlign: 'right' }}>
                          {isLoading ? <Skeleton variant="text" width={80} /> : formatCurrency(serviceProfit)}
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
                        <Typography variant="body1" sx={{ textAlign: 'left' }}>Shipping</Typography>
                        <Typography variant="body1" color="text.primary" sx={{ textAlign: 'right' }}>
                          {isLoading ? <Skeleton variant="text" width={80} /> : formatCurrency(shippingProfit + shippingLoadsProfit)}
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1.5 }}>
                        <Typography variant="body1" sx={{ textAlign: 'left' }}>Finance</Typography>
                        <Typography variant="body1" color="text.primary" sx={{ textAlign: 'right' }}>
                          {isLoading ? <Skeleton variant="text" width={80} /> : formatCurrency(financeProfit)}
                        </Typography>
                      </Box>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
                        ...
                        <Typography variant="h6" fontWeight="bold" color="text.primary" sx={{ textAlign: 'right' }}>
                          {isLoading ? <Skeleton variant="text" width={100} /> : formatCurrency(totalGrossProfit)}
                        </Typography>
                      </Box>
                    </Box>
                  </Box>
                  ...
                  <Box>
                    ...
                    <Box sx={{ pl: 3 }}>
                      {isLoading ? (
                        <>
                          <LoadingSection title="Sales Department" />
                          <LoadingSection title="Service Department" />
                          <LoadingSection title="Shipping Department" />
                          <LoadingSection title="Finance Department" />
                          <LoadingSection title="Non-Tracked Purchases" />
                          <LoadingSection title="NTO Financing" />
                        </>
                      ) : (
